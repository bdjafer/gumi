...
)
{
    ...
    /*
     * The SD card and MCUboot secondary SPI NOR share SPI3. Keep the SD
     * regulator asserted after unmount, matching stock Omi's handoff. Driving
     * it low leaves the shared bus unable to perform the first real external
     * flash erase even though read-only image-state and an already-empty erase
     * request can still appear healthy.
     */
    if (regulator_asserted) {
        cleanup_error = gpio_pin_set_dt(&gumi_sd_enable, 1);
        if (error == 0 && cleanup_error < 0) {
            error = cleanup_error;
        }
    }
