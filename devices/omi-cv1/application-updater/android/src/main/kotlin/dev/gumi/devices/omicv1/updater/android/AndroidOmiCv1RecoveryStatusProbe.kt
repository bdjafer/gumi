...
/** Android BLE adapter for the read-only recovery evidence port. */
@SuppressLint("MissingPermission")
@Suppress("DEPRECATION")
internal class AndroidOmiCv1RecoveryStatusProbe(
    context: Context,
    ...
    override suspend fun inspect(endpoint: EndpointCandidate): OmiCv1RecoveryStatusEvidence {
        if (!applicationContext.hasRecoveryGattPermission()) {
            throw failure(
                OmiCv1ApplicationUpdateFailureCode.PERMISSION_DENIED,
                "Android Nearby Devices permission is required for recovery validation",
            )
        }
        ...
        val device = directory.resolve(endpoint) ?: throw failure(
            ...
            "The process-local Omi endpoint expired before recovery validation",
        )
        return try {
            withTimeout(RECOVERY_GATT_TIMEOUT_MILLIS) {
                inspect(device)
            }
        } catch (error: TimeoutCancellationException) {
            throw failure(
                OmiCv1ApplicationUpdateFailureCode.TRANSPORT_FAILED,
                "Recovery GATT validation timed out",
                error,
            )
        }
    }

    private suspend fun inspect(device: BluetoothDevice): OmiCv1RecoveryStatusEvidence =
            ...
            fun complete(statusBytes: ByteArray) {
                ...
                val result = runCatching {
                    OmiCv1RecoveryStatusProtocol.validate(
                        ...
                        stockIdentityServiceHasCharacteristics = stockIdentityServiceHasCharacteristics,
                    )
                }
                closeConnection(connection)
            ...
            val callback = object : BluetoothGattCallback() {
                override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
                    if (status != BluetoothGatt.GATT_SUCCESS) {
                        fail("Recovery GATT connection failed with status $status")
                    } else if (newState == BluetoothProfile.STATE_CONNECTED) {
                        if (!gatt.discoverServices()) fail("Recovery GATT service discovery did not start")
                    } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                        fail("Recovery GATT disconnected before evidence was read")
                    }
                }

                override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
                    if (status != BluetoothGatt.GATT_SUCCESS) {
                        fail("Recovery GATT service discovery failed with status $status")
                        return
                    }
                    ...
                    stockIdentityServiceHasCharacteristics = identity?.characteristics?.isNotEmpty() == true
                    val characteristic = gatt.getService(RECOVERY_SERVICE_UUID)
                        ?.getCharacteristic(STATUS_CHARACTERISTIC_UUID)
                    if (characteristic == null) {
                        fail(
                            "Recovery status characteristic is absent",
                    ...
                    if (characteristic.properties and BluetoothGattCharacteristic.PROPERTY_READ == 0) {
                        fail(
                            "Recovery status characteristic is not readable",
                            OmiCv1ApplicationUpdateFailureCode.RECOVERY_EVIDENCE_REJECTED,
                        )
                        return
                    }
                    if (!gatt.readCharacteristic(characteristic)) {
                        fail("Recovery status read did not start")
                    }
                }
                ...
                override fun onCharacteristicRead(
                    ...
                    if (status == BluetoothGatt.GATT_SUCCESS) {
                        ...
                        fail("Recovery status read failed with status $status")
                    }
                }
            ...
            try {
                ...
                if (completed.compareAndSet(false, true) && continuation.isActive) {
                    continuation.resumeWithException(
                        failure(
                            OmiCv1ApplicationUpdateFailureCode.PERMISSION_DENIED,
                            "Android rejected recovery GATT permission",
                            error,
                        ),
                    )
                }
    ...
    private companion object {
        val RECOVERY_SERVICE_UUID: UUID = UUID.fromString(OmiCv1RecoveryStatusProtocol.RECOVERY_SERVICE_UUID)
        val STATUS_CHARACTERISTIC_UUID: UUID =
            UUID.fromString(OmiCv1RecoveryStatusProtocol.STATUS_CHARACTERISTIC_UUID)
        val STOCK_IDENTITY_SERVICE_UUID: UUID =
            UUID.fromString(OmiCv1RecoveryStatusProtocol.STOCK_IDENTITY_SERVICE_UUID)
        const val RECOVERY_GATT_TIMEOUT_MILLIS = 15_000L
    }
}

private fun Context.hasRecoveryGattPermission(): Boolean =
