...
| Kotlin Multiplatform `2.4.10` + coroutines/Flow `1.11.0` | Use for the M1 edge SDK/runtime and portable Omi driver logic | Separate Android and Raspberry Pi cores; new event/concurrency primitives | Android and Linux/JVM bootstrap is proven; preserve common-source and import checks as real ports land |
| Native Kotlin + Jetpack Compose BOM `2026.06.01` | Use for the Android shell | Flutter bridge and duplicated lifecycle ownership | Diagnostic APK builds; UI must continue consuming projections/use cases while BLE, durability, and cloud correctness stay outside UI |
...
| [Nordic Android Device Manager `2.8.0`](https://github.com/NordicSemiconductor/Android-nRF-Connect-Device-Manager/tree/559724446b113f46fc60324df4dfd1160faa2a02) | Use behind separate Android image-inspection and guarded firmware-update ports | BLE SMP framing, image state/list commands, upload windowing, confirm/reset state machine | The inspection port exposes only image-list and discloses its transient CCCD/request writes; the future updater must call the single-image path for image `0`, use `CONFIRM_ONLY`, compare image `1` before/after, and never pass a multi-image ZIP to the canary |
